...
(function() {
  var Account, Assignment, Course, L, cheerio, colors, config, http, logger, logging, models, querystring, _,
  ...
  module.exports = {
    ...
    refresh: function(token, options, cb) {
      ...
      return this._parse_courses(token, function(new_token, courses) {
        ...
        parse_course = function(course_data, course_callback) {
          return _this.authenticated_request(token, "course-detail.php?course_id=" + course_data.id, function(err, new_token, $) {
            token = new_token;
    ...
    _parse_courses: function(token, callback) {
      return this.authenticated_request(token, "homework.php", function(err, new_token, $) {
    ...
    authenticated_request: function(token, resource, callback) {
          ...
          if ($('a[href="/students/?Action=logout"]').length === 0) {
            ...
            return _this.authenticate(token.username, token.password, function(err, res) {
              if (err) {
                ...
                return _this.authenticated_request(res.token, resource, callback);
              }
            });
          } else {
