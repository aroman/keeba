...
(function() {
  var MongoStore, RedisStore, app, browserCheck, color, colors, connect, cookie_parser, cp, ensureSession, express, hbpc, http, hydrateSettings, io, jbha, logger, logging, mode, mongo_uri, os, pkg_info, port, pub, redis, redis_pass, redis_url, secrets, server, sessionStore, socketio, ss, ssockets, store, sub, url, workers, _;
  ...
  pub.auth(redis_pass, function() {
    return console.log("Pub UP!");
  });

  sub.auth(redis_pass, function() {
    return console.log("Sub UP!");
  });

  store.auth(redis_pass, function() {
    return console.log("Store UP!");
  });
  ...
  sessionStore = new MongoStore({
    ...
    server.listen(port);
    return logger.info("Keeba " + pkg_info.version + " serving in " + mode[color] + " mode on port " + (port.toString().bold) + ".");
  });
  ...
  ss.on("connection", function(err, socket, session) {
    ...
    socket.on("course:update", function(data, cb) {
      ...
      jbha.Client.update_course(token, data, function() {});
    ...
    socket.on("course:delete", function(data, cb) {
      ...
      jbha.Client.delete_course(token, data, function() {});
    ...
    socket.on("assignments:update", function(data, cb) {
      jbha.Client.update_assignment(token, data, function() {});
    ...
    socket.on("assignments:delete", function(data, cb) {
      ...
      jbha.Client.delete_assignment(token, data, function() {});
